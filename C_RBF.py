...
class DeepComplexRBF(nn.Module):
    def __init__(self, input_dim, hidden_neurons, output_dim):
        ...
        self.m_weights = [torch.zeros_like(i) for i in self.weights]
        self.v_weights = [torch.zeros_like(i) for i in self.weights]

        self.m_biases  = [torch.zeros_like(i) for i in self.biases]
        self.v_biases  = [torch.zeros_like(i) for i in self.biases]

        self.m_centers = [torch.zeros_like(i) for i in self.centers]
        self.v_centers = [torch.zeros_like(i) for i in self.centers]

        self.m_sigmas  = [torch.zeros_like(i) for i in self.sigmas]
        self.v_sigmas  = [torch.zeros_like(i) for i in self.sigmas]
...
################## TRAIN AND SET PARAMETERS ##################
    def training_step(self, x, target, lr):
        ...
        for l in range(L, 0, -1):
            ...
            Gamma.data = Gamma.data + lr_gamma * dG
            ...
            sigma.data = sigma.data + lr_sigma * dS    
...
def prediction_plot(model, X_val, true_values, freq_str):
    ...
    with torch.no_grad():
        ...
        for i in range(0, len(X_val), 5):
            ...
            true_elevation.append(true_values.iloc[i, 1])  # degrees
        ...
        plt.figure()
        plt.scatter(true_elevation, predicted_elevation, color='blue', alpha=0.6)
        ...
        plt.title('Predicted vs. True Elevation')
        plt.legend()
        plt.grid(True)
        plt.savefig(store_path)
...
for freq in frequencies:
    ...
    train_file_path = f"../OneDrive_2_4-22-2025/SNR5_Model2 - 1.0 Az and 15 Elev Spacing FEKO/SNR5_ArrayManifold_UTD_Model2_FEKO_Freq{freq_str}MHz_2024_09_11.txt"
    ...
    for epoch in range(num_epochs):
        ...
        for i in range(0, len(X_train), sample_stride):
            ...
            loss = model.training_step(x_sample, y_target, learning_rate)
    ...
    store_path = f'Results/1_Training_Validation_Loss_FEKO_{freq_str}.png'
    # Plot
    plt.figure()
